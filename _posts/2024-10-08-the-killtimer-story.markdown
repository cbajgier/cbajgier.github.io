...
For whatever reason, I couldn't grasp the mental model that object-oriented programming at that level required. There was an example within the book of a traditional C Windows "Hello, world!" program, meant to showcase the cryptic nature of engaging the Windows API directly and all the overhead involved. This example, with an ongoing message loop, I could understand, and from this single example I was able to write the first version of _KillTimer_. From an old LiveJournal post of mine:

> I had been reading a lot of AOL's online bulletin boards at the time, mostly programming boards, and I remember reading a particularly heated discussion on a C board that dealt with how legitimate such an "anti-idle" program was. It had started with a post along the lines of asking the C code to search for a handle of a particular window, some of the regulars there jumped on the original poster, saying he was asking about how to make an AOL add-on. These types of topics were discouraged.
>
> For whatever reason, I came away incensed that night and had intended on translating my small VB anti-idle program to C and posting the source code on the forum, just to spite the original poster's attackers. I looked into the simplest type of interface possible then, and found that I could just create a "dummy" window and a simple taskbar tray icon rather easily. I got the program written in one night, it simply just sat in the tray with no menu; you just started the program and double clicked the icon to quit.
